...
#need a try catch for if no previous UFC fights...
    ...
    for i, v in npf.iterrows():
        if str(v['WinLoss']) == "win":
            try:
                ...
                if g.group(1) != None:
                    if int(v['Round']) == 1: resultWt= Decimal(resultWt) + Decimal(10.0)
                    if int(v['Round']) == 2: resultWt= Decimal(resultWt) + Decimal(8.0)
                    if int(v['Round']) == 3: resultWt= Decimal(resultWt) + Decimal(4.0)
                    if int(v['Round']) == 4: resultWt= Decimal(resultWt) + Decimal(3.8)
                    if int(v['Round']) == 5: resultWt= Decimal(resultWt) + Decimal(3.8)
            except:
                resultWt= Decimal(resultWt) + Decimal(3.0) 
        else:
            resultWt= Decimal(resultWt) - Decimal(5.0)
...
def getTendencyWts(name,pf,howMany):
    ...
    npf = pf[0:howMany]
##    npf = pf[1:howMany] #BACKTESTING
    stkWt = (Decimal(sum(npf['Strike']))*Decimal(0.5))/Decimal(len(npf))
    tdWt = (Decimal(sum(npf['TakeDowns']))*Decimal(5.00))/Decimal(len(npf))
    subWt = (Decimal(sum(npf['SubAtts']))*Decimal(0.0025))/Decimal(len(npf))
    passWt = (Decimal(sum(npf['GPasses']))*Decimal(3.00))/Decimal(len(npf))
...
##    print("Reach Wt: " + str(reachWt))
    EndMethodWt = getEndMethodWt(name, pf, 4)   #how_many previous fights to look at
##    print("End Method Wt: " + str(EndMethodWt))
    TendencyWt = getTendencyWts(name, pf, 4)   #how_many previous fights to look at
##    print("Tendency Wt: " + str(TendencyWt))
    ELOwt = getELOWt(name,pf,4)
    ...
    try:
        ...
        ##    print(soup.prettify())
            try:
                ...
                reachWt, EndMethodWt, TendencyWt, ELOwt = 0, 0, 0, 0
        else:
            reachWt, EndMethodWt, TendencyWt, ELOwt = 0, 0, 0, 0
    except:
       vitaldata, reachWt, EndMethodWt, TendencyWt, ELOwt = None, 0, 0, 0, 0
...
def InAndOutDK():
    ...
    for i, v in data.iterrows():
        ...
        data.loc[i,'FWeight'] = (float(ELOwt) + float(TendencyWt) + float(EndMethodWt))

        #Salary weighted
##        data.loc[i,'SalaryWeighted'] = float(data.loc[i, 'Salary']) / (float(ELOwt) + float(TendencyWt) + float(EndMethodWt))
...
##    data = data[['DKID','Salary','ELOwt','TendencyWt','EndMethodWt','reachWt','SalaryWeighted','Fighter','Opponent']]
    data = data[['DKID','Salary','ELOwt','TendencyWt','EndMethodWt','reachWt','FWeight','Fighter','Opponent']]
...
##    data = data.sort_values(by=['SalaryWeighted'], ascending=False)
    data = data.sort_values(by=['FWeight'], ascending=False)
##    data = data.sort_values(by=['ELOwt','TendencyWt','EndMethodWt'], ascending=False)
##    dt = date.today()
...
def InAndOutCombinations():
    ...
    finalcombs = pd.DataFrame(columns=[0,1,2,3,4,5,"Conflict","Salary",'ELOwt','TendencyWt','EndMethodWt','reachWt','FWeight'])
    goodOnes = 0
    for i, v in combs.iterrows():
        ...
        if(int(conflict) == 0 and float(sal)<=50000):
            ...
            finalcombs.loc[goodOnes,'FWeight'] = float(fwt)
##            finalcombs.loc[goodOnes,'SalaryWeighted'] = float(salwt)
...
##    finalcombs = finalcombs.sort_values(by=['ELOwt','TendencyWt','EndMethodWt'], ascending=False)
    finalcombs = finalcombs.sort_values(by=['FWeight'], ascending=False)

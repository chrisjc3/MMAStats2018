...
def PrintFighterTable(name, VIT_data, OV_data, PF_data):
    ...
    curlen = len(VIT_data.index)+ len(VIT_data.index) +2
    STKOT_data.to_excel(writer,sheet_name='Sheet1',startrow=curlen, startcol=0, index=False)

    curlen = len(VIT_data.index)+ len(VIT_data.index)+ len(STKOT_data.index) +3
    TD_data.to_excel(writer,sheet_name='Sheet1',startrow=curlen, startcol=0, index=False)

    curlen = len(VIT_data.index)+ len(VIT_data.index)+ len(STKOT_data.index)+ len(TD_data.index) +4
    OT_data.to_excel(writer,sheet_name='Sheet1',startrow=curlen, startcol=0, index=False)

    curlen = len(VIT_data.index)+ len(VIT_data.index)+ len(STKOT_data.index)+ len(TD_data.index)+ len(OT_data.index) +5
...
name = "Tyron Woodley"
html = getNamedFighterHTML(name)
VIT_data = getVITStatsfromHTML(html, name)
OV_data = getOVStatsfromHTML(html, name)
